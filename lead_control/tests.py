...
from lead_control.bitrix_api import BitrixAPIError, create_bitrix_task
...
class LeadControlBitrixAPITests(SimpleTestCase):
    ...
    def test_create_task_updates_when_binding_missing_after_create(self, post_mock):
        ...
        self.assertEqual(
            ...
            {"taskId": 321, "fields": {"UF_CRM_TASK": ["D_555"]}},
        )

    @patch("lead_control.bitrix_api._post")
    def test_create_task_raises_when_binding_still_missing_after_update(self, post_mock):
        post_mock.side_effect = [
            {"result": 321},
            {"result": {"task": {"id": "321", "ufCrmTask": []}}},
            {"result": True},
