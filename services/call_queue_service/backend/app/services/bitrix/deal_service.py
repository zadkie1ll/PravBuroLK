...
def _get_bitrix_timezone() -> ZoneInfo:
    timezone_name = settings.call_queue_bitrix_time_zone or "Europe/Moscow"
    try:
...
class BitrixDealService:
    ...
    def fetch_deals(self, filters: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if filters.get("stage_id"):
            ...
            default_stage = (
                settings.call_queue_bitrix_deal_unanswered_stage_id
                if entity_type == CallEntityType.DEAL
                else settings.call_queue_bitrix_lead_unanswered_status_id
            )
    ...
    def append_deal_comment(self, deal_id: int, line: str) -> str:
        ...
        self.client.call(
            "crm.deal.update",
            {"id": int(deal_id), "fields": {"COMMENTS": new_comments}},
        )
        return new_comments
    ...
    def append_entity_comment(self, entity_type: str, entity_id: int, line: str) -> str:
        if entity_type == CallEntityType.LEAD:
            ...
            self.client.call(
                "crm.lead.update",
                {"id": int(entity_id), "fields": {"COMMENTS": new_comments}},
            )
            return new_comments
    ...
    def extract_contact_phones(self, contact: dict[str, Any]) -> list[str]:
        ...
        for entry in raw_phone:
            ...
            if value:
                ...
                priority = {"MOBILE": 0, "WORK": 1, "OTHER": 2, "HOME": 3, "FAX": 4}.get(value_type, 9)
    ...
    def extract_contact_raw_phones(self, contact: dict[str, Any]) -> list[str]:
        ...
        if not isinstance(raw_phone, list):
            return []
        return [str((entry or {}).get("VALUE") or "").strip() for entry in raw_phone if (entry or {}).get("VALUE")]
    ...
    def extract_entity_phones(self, entity: dict[str, Any]) -> list[str]:
        ...
        if not isinstance(raw_phone, list):
            return []
        return [value for entry in raw_phone if (value := _normalize_phone((entry or {}).get("VALUE") or ""))]
    ...
    def update_entity_after_call(self, entity_type: str, entity_id: int, result: str, repeat_unanswered: bool):
        fields: dict[str, Any] = {}
        # Поля-кастомники Bitrix для этих флагов у нас пока не настроены (нет в .env) —
        # тогда просто пропускаем синк, как и в оригинальном Django-модуле.
        last_call_field = ""
        repeat_field = ""
        if last_call_field:
    ...
    def build_entity_url(self, entity_type: str, entity_id: int | None) -> str:
        if not entity_id:
            return ""
        base_url = (settings.bitrix_base_url or "").rstrip("/")
        if not base_url:
            webhook = settings.bitrix_webhook_url or ""
            base_url = webhook.split("/rest/")[0].rstrip("/") if "/rest/" in webhook else ""
        if not base_url:
            return ""
