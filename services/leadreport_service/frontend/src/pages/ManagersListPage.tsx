...
export function ManagersListPage() {
  ...
  function load() {
    setError("");
    api
      .managersList({ search, is_active: activeFilter === "" ? undefined : activeFilter === "true", page })
      .then(setData)
  ...
  async function toggleActive(row: SalesManagerAdminRow) {
    try {
      await api.patchManagerActive(row.id, !row.is_active);
      load();
    } catch (err) {
  ...
  async function syncFromBitrix() {
    ...
    try {
      const result = await api.syncManagers();
      setSyncMessage(
